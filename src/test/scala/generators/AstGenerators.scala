...
trait AstGenerators extends CommonGenerators {
  ...
  /** Generates a random numeric literal for use in abstract syntax tree (AST) testing.
    *
    * This method creates a generator for `NumericLiteral[Int]` instances. It is used to generate random numeric
    ...
    * @return
    *   A generator that produces instances of `NumericLiteral[Int]`, each containing a randomly generated integer.
    * @see
    *   [[cl.ravenhill.scum.generators.CommonGenerators.generateLong]]
    */
  def generateNumericLiteral(value: Gen[Long] = generateLong()): Gen[NumericLiteral[Int]] = for {
    v <- value
    n <- Gen.const(NumericLiteral[Int](v))
  } yield n

  def generateIdLiteral(environment: Environment): Gen[IdLiteral[Int]] = for {
  ...
  def generateDecrement(maxDepth: Int = 10, environment: Environment): Gen[Decrement[Int]] = for {
    expr <- generateExpression(maxDepth - 1, environment)
    dec  <- Gen.const(Decrement(expr))
  } yield dec

  def generateLet(maxDepth: Int = 10, environment: Environment): Gen[Let[Int]] = for {
  ...
  def generateIf(maxDepth: Int = 10, environment: Environment): Gen[If[Int]] = for {
  ...
  def generateFunction(maxDepth: Int = 10, environment: Environment): Gen[Expression[Int]] =
    Gen.frequency(
