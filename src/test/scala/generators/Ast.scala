...
extension (gen: Gen.type) {
   ...
   */
  def num(value: Gen[Int] = Gen.int()): Gen[ast.terminal.Num] =
    gen.const(ast.terminal.Num(value.sample.get))
   ...
   *   generation.
   */
  def terminal: Gen[ast.terminal.Num] = gen.num()
   ...
   */
  def increment(maxDepth: Int = 5): Gen[ast.unary.Increment] =
   ...
   */
  def decrement(maxDepth: Int = 5): Gen[ast.unary.Decrement] =
   ...
   */
  def let(maxDepth: Int = 5): Gen[ast.Let] =
    gen.const(
      ast.Let(
   ...
   */
  def function(maxDepth: Int = 5): Gen[ast.Expr] =
   ...
   */
  def expr(maxDepth: Int = 5): Gen[ast.Expr] = maxDepth match {
    case 0 => gen.terminal
