...
extension (gen: Gen.type) {
  ...
  /** Generates an instance of the `Num` class with a potentially randomized numeric value.
    ...
    */
  def num(value: Gen[Int] = Gen.int()): Gen[ast.Num[String]] = for {
    v <- value
    n <- Gen.const(ast.Num[String](v))
  } yield n
  ...
  /** Generates an instance of the `Num` class from the `ast.terminal` package.
    ...
    *   generation.
    */
  def terminal: Gen[ast.Expression[String]] = gen.oneOf(gen.num(), gen.varExpr())
  ...
  /** Generates a random instance of a function expression, including unary operations and let bindings, within the
    ...
    */
  def function(maxDepth: Int = 10): Gen[ast.Expression[String]] =
    gen.frequency((1, increment(maxDepth)), (1, decrement(maxDepth)), (3, let(maxDepth)), (2, ifExpr(maxDepth)))

  /** Generates an instance of the [[ast.Expression Expr]] class from the [[ast]] package, representing an expression.
    *
    * This method extends the [[Gen]] object to create a generator for ``Expr`` objects, which represent expressions in
    * a simple expression language. The generator is capable of producing both simple terminal expressions (such as
    ...
    */
  def expr(maxDepth: Int = 10): Gen[ast.Expression[String]] = maxDepth match {
    case 0 => gen.terminal
